/**
 * Autogenerated by soa-sdk-toolkit
 *
 * DO NOT EDIT DIRECTLY
 */
package com.ctrip.soa.global.api.flight.union_searchflightsws.v1;

import com.ctriposs.baiji.rpc.common.apidoc.FieldDoc;
import com.fasterxml.jackson.annotation.JsonAutoDetect;
import com.fasterxml.jackson.annotation.JsonAutoDetect.Visibility;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.google.common.base.MoreObjects;
import com.google.common.base.Objects;
import java.math.BigDecimal;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;

@JsonAutoDetect(getterVisibility = Visibility.NONE, setterVisibility = Visibility.NONE, isGetterVisibility = Visibility.NONE)
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "OriginalPriceDetail", propOrder = {
    "currency",
    "salePrice",
    "publishPrice",
    "accountPrice",
    "tax",
    "fuelSurCharge",
    "settlementPrice"
})
@SuppressWarnings("all")
public class OriginalPriceDetail  {

    private static final long serialVersionUID = 1L;

    public OriginalPriceDetail(
        String currency, 
        BigDecimal salePrice, 
        BigDecimal publishPrice, 
        BigDecimal accountPrice, 
        BigDecimal tax, 
        BigDecimal fuelSurCharge, 
        BigDecimal settlementPrice) {
        this.currency = currency;
        this.salePrice = salePrice;
        this.publishPrice = publishPrice;
        this.accountPrice = accountPrice;
        this.tax = tax;
        this.fuelSurCharge = fuelSurCharge;
        this.settlementPrice = settlementPrice;
    }

    public OriginalPriceDetail() {
    }

    @FieldDoc("Original currency")
    @JsonProperty("Currency")
    @XmlElement(name = "Currency")
    private String currency;

    @FieldDoc("Sales price")
    @JsonProperty("SalePrice")
    @XmlElement(name = "SalePrice", nillable = true)
    private BigDecimal salePrice;

    @FieldDoc("Publish price")
    @JsonProperty("PublishPrice")
    @XmlElement(name = "PublishPrice", nillable = true)
    private BigDecimal publishPrice;

    @FieldDoc("Airline account price")
    @JsonProperty("AccountPrice")
    @XmlElement(name = "AccountPrice", nillable = true)
    private BigDecimal accountPrice;

    @FieldDoc("Tax")
    @JsonProperty("Tax")
    @XmlElement(name = "Tax", nillable = true)
    private BigDecimal tax;

    @FieldDoc("FuelSur Charge")
    @JsonProperty("FuelSurCharge")
    @XmlElement(name = "FuelSurCharge", nillable = true)
    private BigDecimal fuelSurCharge;

    @FieldDoc("Flight Business Department and Vacation settlement price, vacation DIY")
    @JsonProperty("SettlementPrice")
    @XmlElement(name = "SettlementPrice", nillable = true)
    private BigDecimal settlementPrice;

    /**
     * Original currency
     */
    public String getCurrency() {
        return this.currency;
    }

    public void setCurrency(String currency) {
        this.currency = currency;
    }

    /**
     * Sales price
     */
    public BigDecimal getSalePrice() {
        return this.salePrice;
    }

    public void setSalePrice(BigDecimal salePrice) {
        this.salePrice = salePrice;
    }

    /**
     * Publish price
     */
    public BigDecimal getPublishPrice() {
        return this.publishPrice;
    }

    public void setPublishPrice(BigDecimal publishPrice) {
        this.publishPrice = publishPrice;
    }

    /**
     * Airline account price
     */
    public BigDecimal getAccountPrice() {
        return this.accountPrice;
    }

    public void setAccountPrice(BigDecimal accountPrice) {
        this.accountPrice = accountPrice;
    }

    /**
     * Tax
     */
    public BigDecimal getTax() {
        return this.tax;
    }

    public void setTax(BigDecimal tax) {
        this.tax = tax;
    }

    /**
     * FuelSur Charge
     */
    public BigDecimal getFuelSurCharge() {
        return this.fuelSurCharge;
    }

    public void setFuelSurCharge(BigDecimal fuelSurCharge) {
        this.fuelSurCharge = fuelSurCharge;
    }

    /**
     * Flight Business Department and Vacation settlement price, vacation DIY
     */
    public BigDecimal getSettlementPrice() {
        return this.settlementPrice;
    }

    public void setSettlementPrice(BigDecimal settlementPrice) {
        this.settlementPrice = settlementPrice;
    }

    @Override
    public boolean equals(Object obj) {
        if (obj == null) return false;
        if (getClass() != obj.getClass()) return false;

        final OriginalPriceDetail other = (OriginalPriceDetail)obj;
        return
            Objects.equal(this.currency, other.currency) && 
            Objects.equal(this.salePrice, other.salePrice) && 
            Objects.equal(this.publishPrice, other.publishPrice) && 
            Objects.equal(this.accountPrice, other.accountPrice) && 
            Objects.equal(this.tax, other.tax) && 
            Objects.equal(this.fuelSurCharge, other.fuelSurCharge) && 
            Objects.equal(this.settlementPrice, other.settlementPrice);
    }

    @Override
    public int hashCode() {
        int result = 1;

        result = 31 * result + Objects.hashCode(this.currency);
        result = 31 * result + Objects.hashCode(this.salePrice);
        result = 31 * result + Objects.hashCode(this.publishPrice);
        result = 31 * result + Objects.hashCode(this.accountPrice);
        result = 31 * result + Objects.hashCode(this.tax);
        result = 31 * result + Objects.hashCode(this.fuelSurCharge);
        result = 31 * result + Objects.hashCode(this.settlementPrice);

        return result;
    }

    @Override
    public String toString() {
        return MoreObjects.toStringHelper(this)
            .add("currency", currency)
            .add("salePrice", salePrice)
            .add("publishPrice", publishPrice)
            .add("accountPrice", accountPrice)
            .add("tax", tax)
            .add("fuelSurCharge", fuelSurCharge)
            .add("settlementPrice", settlementPrice)
            .toString();
    }
}